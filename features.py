...
def get_time_features(self):
    ...
    for k in range(int(len(self)/15)):
        ...
        for j in range(6):
            ...
            range_ft.append(np.max(self[start:stop,j] - np.min(self[start:stop,j])))
            std_ft.append(np.std(current_row))
    ...
    range_ft = np.reshape(range_ft,(k+1, j+1))
    std_ft = np.reshape(std_ft,(k+1,j+1))
    # mode_ft = np.reshape(mode_ft,(k+1,j+1))
    time_stats = np.c_[mean, minimum, maximum, median_ft,std_ft, range_ft];#, mode_ft]
